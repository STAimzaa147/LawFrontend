...
export default function CategoryMenu({ currentCategory }: CategoryMenuProps) {
  ...
  useEffect(() => {
    fetchCategoriesFromNews()
  }, [])

  const fetchCategoriesFromNews = async () => {
    try {
      // Fetch all news to extract categories
      ...
      if (data.success) {
        ...
        // Extract and count categories from news items
      ...
      console.error("Error fetching categories from news:", error)
