...
export default function AdminForumReportsPage() {
  ...
  const fetchReports = async () => {
    ...
    try {
      ...
      const response = await fetch(`${backendUrl}/api/v1/report/forum`, {
        method: "GET",
        headers: {
      ...
      if (data.success) {
        ...
        const sortedReports = data.data.sort((a: ForumReport, b: ForumReport) => {
        ...
        toast({
          title: "ข้อผิดพลาด",
          description: "ไม่สามารถดึงข้อมูลรายงานกระทู้ได้",
          variant: "destructive",
        })
      }
    } catch (error: any) {
  ...
  const handleResolveReport = async (reportId: string) => {
    ...
    try {
      ...
      const response = await fetch(`${backendUrl}/api/v1/report/forum/${reportId}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          "Authorization": `Bearer ${session.accessToken}`,
  ...
  if (loading) {
    return <div className="text-center py-8 text-white">กำลังโหลดรายงานกระทู้...</div>
  }

  return (
    <Card className="bg-white rounded-xl shadow-lg">
      <CardHeader>
        <CardTitle className="text-gray-900">รายการรายงานกระทู้</CardTitle>
      </CardHeader>
      <CardContent>
        {reports.length === 0 ? (
          <p className="text-center text-gray-500">ไม่พบรายงานกระทู้</p>
        ) : (
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-gray-50">
                  ...
                  <TableHead className="text-gray-600">หัวข้อกระทู้</TableHead>
              ...
              <TableBody>
                {reports.map((report) => (
                  <TableRow key={report._id} className="border-b border-gray-200 hover:bg-gray-50">
                    ...
                    <TableCell className="text-gray-800">{report.forum_id?.title || "กระทู้ถูกลบแล้ว"}</TableCell>
                    ...
                    <TableCell className="text-gray-800">{report.status === "pending" ? "รอดำเนินการ" : "แก้ไขแล้ว"}</TableCell>
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="bg-white text-gray-900">
                          <DropdownMenuItem asChild>
                            <Link
                              href={`/admin/reports/forums/${report._id}`}
